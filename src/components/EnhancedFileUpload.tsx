...
const EnhancedFileUpload: React.FC<EnhancedFileUploadProps> = ({ onFileUpload, isProcessing, error }) => {
  ...
  // Show processing summary if completed
  if (processingCompleted && statistics) {
    return (
      <ProcessingSummary
        statistics={statistics}
        isCompleted={true}
        hasErrors={statistics.errorCount > 0}
        onViewDetails={() => setShowDetailedStats(true)}
        onDownloadReport={() => {
  ...
  if (showDetailedStats && statistics) {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      {(isWorkerProcessing || isChunkedProcessing || isProcessing) && (
        <div className="space-y-4">
          {/* Worker Status */}
          {isWorkerProcessing && (
            <WorkerStatus
          ...
          <div className="mt-6 space-y-4">
            ...
            {validationResult.warnings.length > 0 && (
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-md">
                <div className="flex items-start">
                  <AlertTriangle className="h-5 w-5 text-yellow-500 mt-0.5 flex-shrink-0 mr-3" />
                  <div>
                    <h3 className="text-yellow-800 font-medium">Warnings ({validationResult.warnings.length})</h3>
      ...
      <CancellationDialog
        ...
        processingStats={statistics ? {
          ...
          processedRows: statistics.processedRows,
          elapsedTime: statistics.currentTime - statistics.startTime,
          estimatedTimeRemaining: statistics.estimatedEndTime 
